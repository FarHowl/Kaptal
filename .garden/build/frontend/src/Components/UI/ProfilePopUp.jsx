...
export default function ProfilePopUp({ profilePopUpRef }) {
    ...
    if (getUserData()) {
        return (
            <div ref={profilePopUpRef} className="absolute scale-0 z-20 top-0 right-0 origin-top-right opacity-0 pointer-events-none bg-white shadow-xl rounded-lg animated-300">
                ...
                <div className="w-[280px] text-base flex flex-shrink-0 flex-col gap-2 justify-center px-3 pb-2 pt-4 items-start">
                    ...
                    <button className="hover:text-blue-500">Заказы</button>
                    <button className="hover:text-blue-500">Настройки профиля</button>
                    <button
                        onClick={() => {
                            logOut();
                            navigate("/");
