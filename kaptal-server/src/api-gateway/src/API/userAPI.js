...
router.get("/users-service/user/getUserData", async (req, res) => {
    try {
        const frontendToken = verifyJWT(req, ["admin", "user", "moderator"]);
...
router.post("/orders-service/user/makeOrder", async (req, res) => {
    try {
        ...
        const response = await axios.post("http://orders-service:3000" + "/api/user/makeOrder", { ...req.body }, { headers: { Authorization: "Bearer " + newToken } });
...
router.get("/orders-service/user/getOrders", async (req, res) => {
    try {
        ...
        const response = await axios.get("http://orders-service:3000" + "/api/user/getOrders", { headers: { Authorization: "Bearer " + newToken } });
